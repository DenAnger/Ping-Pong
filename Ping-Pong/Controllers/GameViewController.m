...
//  Ping-Pong
//
//  Created by Denis Abramov on 02/02/2019.
...
- (void)config {
    self.view.backgroundColor = [UIColor colorWithRed: 100.0/255.0
                                                green: 135.0/255.0
                                                 blue: 191.0/255.0
                                                alpha: 1.0];
    _gridView = [[UIView alloc] initWithFrame: CGRectMake(0,
    ...
    _scoreTop.text = @"0";
    _scoreTop.font = [UIFont systemFontOfSize: 40.0
    ...
    _scoreBottom.text = @"0";
    _scoreBottom.font = [UIFont systemFontOfSize: 40.0
                                          weight: UIFontWeightLight];
...
- (void)displayMessage: (NSString *) message {
    ...
    UIAlertController *alertController = [
                                          UIAlertController alertControllerWithTitle: @"Ping Pong"
                                          message: message
...
- (BOOL)goal {
    if (_ball.center.y < 0 || _ball.center.y >= SCREEN_HEIGHT) {
        ...
        if (gameOver) {
            [self displayMessage: [NSString stringWithFormat: @"Player %i win",
                                   gameOver]];
        } else {
            [self reset];
        }
...
- (void)animate {
    _ball.center = CGPointMake(_ball.center.x + _dx*_speed,
                               _ball.center.y + _dy*_speed);
    [self checkCollision: CGRectMake(0, 0, 20, SCREEN_HEIGHT)
                       X: fabs(_dx)
                       Y: 0];
    [self checkCollision: CGRectMake(SCREEN_WIDTH, 0, 20, SCREEN_HEIGHT)
                       X: -fabs(_dx)
                       Y: 0];
    if ([self checkCollision: _paddleTop.frame
                           X: (_ball.center.x - _paddleTop.center.x) / 32.0
                           Y: 1]) {
        [self increaseSpeed];
    }
    if ([self checkCollision: _paddleBottom.frame
                           X: (_ball.center.x - _paddleBottom.center.x) / 32.0
                           Y: -1]) {
        [self increaseSpeed];
    }
    [self goal];
